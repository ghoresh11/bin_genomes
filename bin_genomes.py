...
def _convert_gffs_to_fasta(genomes_file, tmp_out, args, log, verbose):
    ...
    with open(genomes_file) as f:
        for line in f:
            ...
            fasta_loc = gff_to_fasta(gff_file, new_loc, qc=args, log=log)
...
def remove_temp(keep_temp, out_dir, verbose):
...
def run(args):
    ...
    remove_temp(args["keep_temp"], out_dir, verbose=args["verbose"])
